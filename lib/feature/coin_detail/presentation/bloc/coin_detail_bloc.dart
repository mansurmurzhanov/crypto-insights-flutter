...
class CoinDetailBloc extends Bloc<CoinDetailEvent, CoinDetailState> {
  ...
  CoinDetailBloc(this.getCoinDetailUseCase) : super(const CoinDetailState()) {
  ...
  Future<void> _onLoadCoinDetail(
    ...
    try {
      final coin = await getCoinDetailUseCase(event.coinId);

      emit(state.copyWith(status: CoinDetailStatus.success, coin: coin));
    } catch (e) {
      emit(
        state.copyWith(
          status: CoinDetailStatus.failure,
          failure: e is Failure ? e : const UnknownFailure('Unexpected error'),
        ),
      );
    }
  }
