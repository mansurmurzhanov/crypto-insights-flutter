...
class CoinsBloc extends Bloc<CoinsEvent, CoinsState> {
  ...
  CoinsBloc(this.getCoinsUseCase) : super(const CoinsState()) {
  ...
  Future<void> _onLoadCoins(LoadCoins event, Emitter<CoinsState> emit) async {
    ...
    try {
      ...
      emit(
        state.copyWith(
          ...
          error: e is Failure ? e : const UnknownFailure('Unexpected error'),
        ),
      );
    }
  }
  ...
  Future<void> _onRefreshCoins(
    ...
    try {
      final coins = await getCoinsUseCase(forceRefresh: true);
  ...
  void _onSearchCoins(SearchCoins event, Emitter<CoinsState> emit) {
    emit(state.copyWith(query: event.query));
  }

  void _onSortCoins(SortCoins event, Emitter<CoinsState> emit) {
    ...
    emit(state.copyWith(coins: sortedCoins, sortBy: event.sortBy));
  }

  void _onLoadMoreCoins(LoadMoreCoins event, Emitter<CoinsState> emit) {
    ...
    emit(
      state.copyWith(
        visibleCount: (state.visibleCount + 20).clamp(0, state.coins.length),
      ),
    );
  }
}
